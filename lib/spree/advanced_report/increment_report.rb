  ...
  def generate_ruport_data
    ...
    INCREMENTS.each do |inc|
      ...
      ruportdata[inc].data.each do |p|
        puts "*** #{inc}"
        all_data << { 'increment' => inc.to_s.capitalize, 'key' => p.data['key'], 'display' => p.data['display'], 'value' => p.data['value'] }
      end
    ...
    all_data.sort_rows_by!(['key'])
    all_data.remove_column('key')
    self.all_data = Grouping(all_data, by: 'increment')
  end
